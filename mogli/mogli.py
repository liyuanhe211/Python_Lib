...
ATOM_VALENCE_RADII = np.array([0,  # Avoid atomic number to index conversion
                               ...
                               1120, 970, 1100, 1200, 1200, 900, 1020, 990,
...
# Atom display radii in Å
ATOM_DISPLAY_RADII = [x**(1/3) for x in list(ATOM_VALENCE_RADII)]
ATOM_DISPLAY_RADII = np.array(ATOM_DISPLAY_RADII,dtype=np.float32)/3
...
# Atom display radii in Å (may be changed by users)
ATOM_DISPLAY_RADII = ATOM_DISPLAY_RADII*1
...
class Molecule(object):
    ...
    def __init__(self, atomic_numbers, positions):
        ...
        self.bonds = None
        self.atomic_radii = ATOM_RADII[self.atomic_numbers]
        self.atomic_display_radii = ATOM_DISPLAY_RADII[self.atomic_numbers]

    def calculate_bonds(self, method='radii', param=None):
         # define bond existance when radii<1.3*sum of valence radii
        ...
        if method == 'radii':
            ...
            if param is None:
                param = 1.3 # define bond existance when radii<1.3*sum of valence radii
...
class Bonds(object):
    ...
    def __str__(self):
        return "\n".join([str(x) for x in self.index_pairs])
...
def show(molecule, width=500, height=500,
    ...
    # gr3.setviewport(0, 0.7, 0, 0.7)
    # gr3.setwindow(0.1, 0.9, 0.05, 0.85)
