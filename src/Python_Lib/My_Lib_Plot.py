...
# Set Qt to offscreen mode BEFORE importing Qt or matplotlib if we're in headless environment
# This must be done before any Qt imports to take effect
...
# ─────────────────────── Human-readable display dictionaries ───────────────────────
MARKER_DISPLAY = {
    '': '',
    'Point (.)': '.', 'Pixel (,)': ',', 'Circle (o)': 'o',
    'Circle Hollow': 'o_HOLLOW',
    'Tri Down (v)': 'v', 'Tri Up (^)': '^',
    'Tri Left (<)': '<', 'Tri Right (>)': '>',
    'Tri Down Alt (1)': '1', 'Tri Up Alt (2)': '2',
    'Tri Left Alt (3)': '3', 'Tri Right Alt (4)': '4',
    'Square (s)': 's', 'Square Hollow': 's_HOLLOW',
    'Pentagon (p)': 'p', 'Star (*)': '*',
    'Hexagon (h)': 'h', 'Hexagon Alt (H)': 'H',
    'Plus (+)': '+', 'X Mark (x)': 'x',
    'Diamond (D)': 'D', 'Thin Diamond (d)': 'd',
    'V-Line (|)': '|', 'H-Line (_)': '_',
}
MARKER_DISPLAY_REV = {v: k for k, v in MARKER_DISPLAY.items()}
...
LINE_FORMAT_DISPLAY_REV = {v: k for k, v in LINE_FORMAT_DISPLAY.items()}

COLOR_DISPLAY = {
    '': '',
    'Blue': 'tab:blue', 'Red': 'tab:red', 'Green': 'tab:green',
    'Purple': 'tab:purple', 'Orange': 'tab:orange', 'Cyan': 'tab:cyan',
    'Olive': 'tab:olive', 'Pink': 'tab:pink', 'Brown': 'tab:brown',
    'Grey': 'tab:grey', 'Black': 'k', 'White': 'w',
}
COLOR_DISPLAY_REV = {v: k for k, v in COLOR_DISPLAY.items()}

# ─────────────────────── Gradient colormap groups ───────────────────────
GRADIENT_COLORMAP_GROUPS = [
    ('Default', ['Tab colors']),
...
GRADIENT_COLORMAPS = [name for _group, names in GRADIENT_COLORMAP_GROUPS for name in names]

# ─────────────────────── Fill color schemes ───────────────────────
# Each scheme is a list of (start, end, color_spec, name) tuples.
# color_spec can be:
#   - A single color string: solid fill with that color
#   - A matplotlib colormap name string (e.g. "gist_rainbow"): gradient fill
#   - A tuple of two color strings: HSV interpolation between those two colors
Solar_wavelengths = [(100, 290, 'black', 'Harmful'),
                     (290, 315, '#7603B2', 'UVB'),
                     (315, 335, '#8900D0', 'UVA2'),
                     (335, 380, '#7200E7', 'UVA1'),
                     (700, 1400, "gist_rainbow", 'IRA'),
                     (1400, 3000, 'red', 'IRB'),
...
WINDOW_POSITIONS = {1: ((0, 0),),
                    ...
                    8: ((-1.5, -0.5), (-0.5, -0.5), (0.5, -0.5), (1.5, -0.5),
                        (-1.5, 0.5), (-0.5, 0.5), (0.5, 0.5), (1.5, 0.5)),
                    9: ((-1.5, -0.5), (-0.5, -0.5), (0.5, -0.5), (1.5, -0.5),
...
def Curve_from_JSON(filename_or_content) -> 'Curve':
    if isinstance(filename_or_content, str) and (os.path.isfile(filename_or_content) or filename_or_content.endswith('.json')):
        ...
        raise ValueError("Invalid input for Curve_from_JSON")
    
    # Remove keys that are not arguments to Curve.__init__? 
    # Actually Curve accepts **kwargs? No, it has explicit args.
    # But Curve_Data mirrors Curve args.
    # Curve_Data has 'X' and 'Y'. Curve uses 'X' and 'Y'.
    # Curve_Data has 'X_and_Y' etc.
    
    # We should instantiate Curve with these parameters.
    return Curve(**data)
...
def Plot_from_JSON(filename_or_content, target_window: 'Plot' = None) -> 'Plot':
    """
    Load a Plot from a JSON file, JSON string or dict.

    Parameters
    ----------
    filename_or_content : str or dict
        Path to a .json file, a JSON string, or a dict.
    target_window : Plot, optional
        If provided, the existing Plot window is *updated in-place*
        (its properties and Curve/Grid objects are replaced) instead of
        creating a brand-new Plot widget.  This avoids window flicker
    ...
    if data.get('Curve_objects'):
        curves = data['Curve_objects']
        if isinstance(curves, list):
            data['Curve_objects'] = [Curve_from_JSON(c) for c in curves]
        else: # Single object
            data['Curve_objects'] = Curve_from_JSON(curves)

    if data.get('Grid_objects'):
        grids = data['Grid_objects']
    ...
    if target_window is not None:
        ...
        # Do NOT update shift_window - it would trigger window movement
        # Only update data and display properties
        w = target_window
        if 'x_axis_label' in data: w.x_axis_label = data['x_axis_label']
        if 'y_axis_label' in data: w.y_axis_label = data['y_axis_label']
        if 'y2_axis_label' in data: w.y2_axis_label = data['y2_axis_label']
        if 'x_lim' in data: w.x_lim = data['x_lim']
        if 'y_lim' in data: w.y_lim = data['y_lim']
        if 'x_log' in data: w.x_log = data['x_log']
        if 'y_log' in data: w.y_log = data['y_log']
        if 'show_grid' in data: w.show_grid = data['show_grid']
        if 'plot_legend' in data: w.plot_legend = data['plot_legend']
        if 'figure_title' in data: w.set_figure_title(data['figure_title'])
...
class Curve:
    ...
    def to_JSON_data(self):
        """
        Convert Curve object to Curve_JSON structure.
        """
        return Curve_DataClass(
            X=self.Xs,
            Y=self.Ys,
            X_label=self.X_label,
            ...
            curve_color=self.curve_color,
            curve_width=self.curve_width,
            do_interpolation=self.do_interpolation,
            interpolation_kind=self.interpolation_kind,
            interpolation_smoothing=self.interpolation_smoothing,
            interpolation_number=self.interpolation_number,
    ...
    def integrate(self, start, stop, weights=None):
        """Integrate the curve between start and stop using linear interpolation.

        Parameters
        ----------
        start, stop : float
            Integration bounds. Order doesn't matter; they will be sorted.
        weights : str, list, or None
            - str: path to CSV/XLSX file with two columns (X, weight).
            - list of 2 lists: [[X_values], [weight_values]].
        ...
        Notes
        -----
        If weights or this Curve's X range does not cover [start, stop],
        the method checks whether negating X covers the range, and if so
        uses the negated X axis for that data.
        """
        start, stop = min(start, stop), max(start, stop)
        N = 10000

        # --- prepare curve data (possibly negate X) ---
        curve_xs = np.array(self.Xs, dtype=float)
        curve_ys = np.array(self.Ys, dtype=float)

        if min(curve_xs) <= start and max(curve_xs) >= stop:
            pass  # normal range covers
        else:
            neg_xs = -curve_xs
            neg_start, neg_stop = min(-stop, -start), max(-stop, -start)
            if min(neg_xs) <= neg_start and max(neg_xs) >= neg_stop:
                curve_xs = neg_xs
                # sort after negation
                sort_idx = np.argsort(curve_xs)
                curve_xs = curve_xs[sort_idx]
                curve_ys = curve_ys[sort_idx]
        ...
        interp_xs = np.linspace(start, stop, N)
        curve_interp = np.interp(interp_xs, curve_xs, curve_ys)
        ...
        if weights is not None:
            if isinstance(weights, str):
                ...
                if ext in ('.xlsx', '.xls'):
                    import pandas as pd
                    df = pd.read_excel(weights, header=None)
                else:
                    import pandas as pd
                    df = pd.read_csv(weights, sep=None, engine='python', header=None)
                w_xs = df.iloc[:, 0].values.astype(float)
                w_ys = df.iloc[:, 1].values.astype(float)
            elif isinstance(weights, (list, tuple)) and len(weights) == 2:
                w_xs = np.array(weights[0], dtype=float)
                w_ys = np.array(weights[1], dtype=float)
            else:
                raise ValueError("weights must be a file path, or a list of 2 lists [[Xs], [Ys]]")

            # possibly negate weights X
            if min(w_xs) <= start and max(w_xs) >= stop:
                pass
            else:
                neg_w_xs = -w_xs
                neg_start_w, neg_stop_w = min(-stop, -start), max(-stop, -start)
                if min(neg_w_xs) <= neg_start_w and max(neg_w_xs) >= neg_stop_w:
                    w_xs = neg_w_xs

            sort_idx = np.argsort(w_xs)
            w_xs = w_xs[sort_idx]
            w_ys = w_ys[sort_idx]
...
class Grid:
    ...
    def to_JSON_data(self):
        """
        Convert Grid object to Grid_JSON structure.
        """
        xyz_triples = None
        if self.Xs is not None and self.Ys is not None and self.Zs is not None:
             xyz_triples = list(zip(self.Xs, self.Ys, self.Zs))

        return Grid_DataClass(
            XYZ_triples=xyz_triples,
            Xs=self.Xs_input,
            Ys=self.Ys_input,
            do_interpolation=self.do_interpolation,
            interpolation_type=self.interpolation_type,
            must_pass_through_points=self.must_pass_through_points,
            smoothing=self.smoothing,
            interpolation_density=self.grid_density,
            Z_axis_colors=self.Z_axis_colors,
            Z_linear_or_log=self.Z_linear_or_log,
            show_contour=self.show_contour,
            contour_levels=self.contour_levels,
            contour_values=self.contour_values,
            show_contour_labels=self.show_contour_values,
            contour_style=self.contour_style,
            contour_width=self.contour_width,
            contour_color=self.contour_color,
            contour_do_interpolation=self.contour_do_interpolation,
            contour_interpolation_type=self.contour_interpolation_type,
            contour_must_pass_through_points=self.contour_must_pass_through_points,
            contour_smoothing=self.contour_smoothing,
            normalize_to=None,
            scale_factor=None,
...
class Plot(QtWidgets.QWidget, Qt_Widget_Common_Functions):
    def __init__(
            ...
            y_axis_label="Y",
            fig_size_inch=(3, 2),
            fig_size_pixel=None,  # (width_px, height_px) to specify the axes area size in pixels. Overrides fig_size_inch/fig_size_inch.
            window_size_pixel=None,  # (width_px, height_px) to specify the window size in pixels. Overrides fig_size_pixel and fig_size_inch.
            font_size=8,
            plot_legend=True,
        ...
        # --- Figure size unification ---
        # Priority: window_size_pixel > fig_size_pixel > fig_size_inch
        # window_size_pixel: fixes the total window size; fig auto-fills available space
        # fig_size_pixel: specifies axes area size in pixels (overrides fig_size_inch)
        # fig_size_inch: default axes area size in inches
        ...
        if window_size_pixel is not None:
            ...
            self._fig_size_pixel = (300, 200)  # Temporary default
        elif fig_size_pixel is not None:
            self._fig_size_pixel = tuple(fig_size_pixel)
        else:
            self._fig_size_pixel = (fig_size_inch[0] * DEFAULT_FIG_DPI, fig_size_inch[1] * DEFAULT_FIG_DPI)

        # Create Figure with approximate size (before HiDPI correction)
        temp_ax_w = self._fig_size_pixel[0] / DEFAULT_FIG_DPI
        temp_ax_h = self._fig_size_pixel[1] / DEFAULT_FIG_DPI
        fig_width = temp_ax_w + 1.2
        fig_height = temp_ax_h + 1.2

        self._fig = Figure(figsize=(fig_width, fig_height), dpi=DEFAULT_FIG_DPI, layout='constrained')
        ...
        label_layout = QtWidgets.QHBoxLayout(self.label_container)
        ...
        if self.plot_objects:
            self.show()
            # If window_size_pixel is specified, resize window to exact size
    ...
    def showEvent(self, a0):
        """Override showEvent to raise window on first show."""
        super().showEvent(a0)
        
        # On first show, raise to front (unless already handled by keep_front)
        if not self._first_show_done:
            self._first_show_done = True
            # Raise window to ensure it's visible on creation
            self.raise_()
            self.activateWindow()
            # Don't keep it on top permanently unless keep_front=True
            # After first show, keep_front flag controls always-on-top behavior
    ...
    @keep_front.setter
    def keep_front(self, value):
        self._keep_front = value
        if value:
            self.setWindowFlags(self.windowFlags() | QtCore.Qt.WindowType.WindowStaysOnTopHint)
        else:
            self.setWindowFlags(self.windowFlags() & ~QtCore.Qt.WindowType.WindowStaysOnTopHint)
    ...
    def _detect_device_pixel_ratio(self):
        """Detect HiDPI scaling factor from canvas and screen."""
        canvas_dpr = 1.0
        dpi_scale = 1.0
        try:
            dpr_getter = getattr(self._canvas, "devicePixelRatioF", None)
            if callable(dpr_getter):
                canvas_dpr = float(dpr_getter())
        except Exception:
            pass
        try:
            if self._app is not None:
                ...
                if screen is not None:
                    ...
                    dpi_scale = logical_dpi / WINDOWS_LOGICAL_DPI_BASE
        except Exception:
            pass
        return max(canvas_dpr, dpi_scale, 1.0)

    def _setup_figure_size(self):
        """Centralized figure size management.

        Converts self._fig_size_pixel (logical screen pixels) to axes size
        in inches, using DEFAULT_FIG_DPI as the consistent conversion factor.

        On HiDPI displays, Qt's matplotlib backend automatically adjusts
        self._fig.dpi to account for device pixel ratio, so we must NOT
        multiply by device_pixel_ratio here — that would double-count.

        Must be called after Figure and Canvas are created.
        """
        # Use DEFAULT_FIG_DPI (100) consistently for pixel↔inch conversion.
        # self._fig.dpi may have been changed by the Qt backend on HiDPI
        # displays (e.g. 100→150), but fig_size_pixel is in logical pixels.
        effective_dpi = DEFAULT_FIG_DPI

        # Target axes area in inches
        ax_width = self._fig_size_pixel[0] / effective_dpi
        ax_height = self._fig_size_pixel[1] / effective_dpi
        self.fig_size_inch = (ax_width, ax_height)

        # Figure = axes + padding; constrained_layout positions the axes,
        # _enforce_axes_size adjusts figure size later to match target.
        fig_width = ax_width + 1.2
    ...
    def _enforce_axes_size(self):
        ...
        try:
            ...
            target_ax_w, target_ax_h = self.fig_size_inch
            fig_w, fig_h = self._fig.get_size_inches()
            ax_bbox = self._ax.get_position()
            current_ax_w = ax_bbox.width * fig_w
            current_ax_h = ax_bbox.height * fig_h
            delta_w = target_ax_w - current_ax_w
            delta_h = target_ax_h - current_ax_h
            if abs(delta_w) > 0.01 or abs(delta_h) > 0.01:
                new_fig_w = fig_w + delta_w
                new_fig_h = fig_h + delta_h
                self._fig.set_size_inches(new_fig_w, new_fig_h)
        except Exception as e:
            print(f"Error enforcing axes size: {e}")

    def resize_window_to_fig(self):
        """Resize the window so that the canvas exactly matches the current figure size.
        ...
        # On HiDPI displays, self._fig.dpi may have been elevated (e.g. 150)
        # by the Qt backend, but setFixedSize expects logical pixels.
        # fig_inches * DEFAULT_FIG_DPI = logical screen pixels.
    ...
    # Aliases so that both Plot.Curve_objects and Plot.Grid_objects
    # go through the same property setter (which triggers _update_plot).
    @property
    def Curve_objects(self):
        return self._plot_objects

    @Curve_objects.setter
    def Curve_objects(self, new_objects):
    ...
    def _update_plot_curve(self):
        ...
        for curve in self._plot_objects:
            ...
            if curve.fill_color is not None:
                self._draw_fill(curve, X, Y)
    ...
    def _draw_fill(self, curve, X, Y):
        """Draw fill between curve and Y=0.
        
        fill_color can be:
          - A color string (e.g. 'tab:blue', '#FF0000'): solid fill with alpha
          - A fill scheme name from FILL_SCHEMES (e.g. 'Solar'): sectioned gradient fill
        """
        ...
        if curve.do_interpolation and curve.plot_curve:
            try:
                fill_X = np.linspace(min(X), max(X), curve.interpolation_number)
                fill_Y = curve.interp1d(fill_X)
            except Exception:
                fill_X, fill_Y = X, Y
        else:
            fill_X, fill_Y = X, Y

        # Check if it's a fill scheme name
        if fill_color in FILL_SCHEMES:
            scheme = FILL_SCHEMES[fill_color]
            for (x_start, x_end, color_spec, _name) in scheme:
                mask = (fill_X >= x_start) & (fill_X <= x_end)
                if not np.any(mask):
                    continue
                section_X = fill_X[mask]
                section_Y = fill_Y[mask]
    ...
    def _fill_with_hsv_gradient(self, X, Y, color1, color2):
        ...
        c2_hsv = np.array(mcolors.rgb_to_hsv(mcolors.to_rgb(color2)))
        n = len(X)
        if n < 2:
            return
        for i in range(n - 1):
    ...
    def save_plot_history(self, output_filename):
        ...
        if "Plot_DataClass" in globals():
            try:
                json_path = os.path.join(plot_history_folder, filename_replace_last_append(filename_name(output_filename),".json.Plot"))
                self.dump_to_JSON(json_path)
            except Exception as e:
                traceback.print_exc()
                print(f"Failed to save plot history JSON: {e}")
    ...
    def to_DataClass(self):
        ...
        return Plot_DataClass(
            ...
            fig_size_pixel=self._fig_size_pixel,
            ...
            x_tick_texts=self.x_tick_texts,
            y_tick_positions=self.y_tick_positions,
            y_tick_texts=self.y_tick_texts,
            y2_tick_positions=self.y2_tick_positions,
            y2_tick_texts=self.y2_tick_texts,
            y2_axis_label=self.y2_axis_label,
...
def substract_component(curve, bkg,
    ...
    def penalty(ratio):
        ...
        # write_xlsx(get_unused_filename('ReactIR/Penalty Check {:.3f}.xlsx'.format(ratio)),monitor_by_xlsx,transpose=True)
...
if "check_consistency" in locals():
    # Validate that JSON data classes match implementation classes
    # This runs on import to ensure definitions stay in sync
    check_consistency(Curve, Grid, Plot)
...
-222.94	115890.4572"""
    ...
    # Plot(my_grid, figure_title="Grid Plot Test", fig_size_inch=(10,6)).pause()
