...
class Coordinates:
    def __init__(self,
                 coordinates=(),
                 charge: Union[int, str] = 999,
                 multiplet: Union[int, str] = 999,
                 selected_atom_range=(),
                 is_xtb=False,
                 periodic_cell=()):
        """
        ...
        self._hashnum: Optional[int] = None
    ...
    def elements_num(self):
        return [element_to_num_dict[x] for x in self.elements]

    @property
    def coordinates_numer(self):
        return [[float(x) for x in coord.split('\t')[1:4]] for coord in self.coordinates]

    def update_coordinates_np(self):
        self.coordinates_np = [np.array(coord) for coord in self.coordinates_numer]
    ...
    # def delete_atom(self,atom_count):
    #     """
    #
    #     :param atom_count: start from 1
    #     :return:
    #     """
    #     atom_count -= 1
    #     self.elements.pop(atom_count)
    #     self.coordinates.pop(atom_count)
    #     self.update()
    #
    # def delete_atoms(self,atom_counts):
    #     """
    #
    #     :param atom_counts: starts from 1
    #     :return:
    #     """
    #     for atom_count in sorted(atom_counts,reverse=True):
    #         self.delete_atom(atom_count)

    def update(self):
        self.update_coordinates_np()
        self._distance_matrix = None
        self._hashnum = None
    ...
    def distance_matrix(self):
        if self._distance_matrix is None:
            self._distance_matrix = [[-1] * self.atom_count for _ in range(self.atom_count)]  # n*n matrix
            ...
            assert -1 not in sum(self._distance_matrix, []), 'Distance Matrix Error'
    ...
    def open_GView(self):
        ...
        save_config(config)
        gjf_file = self.gjf_file()
        print("Opening:",gjf_file)
        subprocess.Popen([gview_exe,gjf_file])
...
def get_dihedral(coordinate, atom_indexes):
    ...
    :param coordinate:
    :param atom_indexes: same as the Dihedral class, start from 0
    :return:
    """
...
class Dihedral:
    def __init__(self, coordinate: Coordinates, atom_indexes):
        ...
        :param coordinate:
        :param atom_indexes:  start from 0, should be left-atom, center_bond1, center_bond2, right-atom.
...
def get_connectivity_from_coordinate(coordinates: Coordinates, bond_radii_factor=1.3, remove_atoms = []):
    ...
    bonds_detected = [x for x in bonds_detected if x[0] not in remove_atoms and x[1] not in remove_atoms]
