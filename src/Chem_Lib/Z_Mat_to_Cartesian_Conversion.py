...
"""
Downloaded and modified, from https://github.com/jevandezande/zmatrix/blob/master/converter.py
A module for converting between zmatrices and cartesian coordinates
"""

#Example Z matrix input (THe first two lines will be dumped
...
C	1	1.511
H	1	1.088	2	110.5
H	1	1.091	2	110.1	3	-108.8
H	1	1.091	2	110.1	3	108.8
O	2	1.431	1	107.8	3	180.0
H	6	0.971	2	105.4	1	180.0
H	2	1.098	1	110.7	3	-59.865
H	2	1.098	1	110.7	3	59.865'''
...
class Converter:
    ...
    def read_zmatrix(self, input_file='zmatrix.dat',origin1 = None, origin2 = None,origin3 = None):
        """
        ...
        :param origin1,origin2,origin3: three np.array(), A three-point origin to determine the cartesian origin and the absolute orientation
    ...
    def add_first_three_to_cartesian(self):
        ...
        if isinstance(self.origin1,np.ndarray):
            name, coords, mass = self.zmatrix[0]
            self.cartesian = [[name, self.origin1, mass]]
        else:
            # First atom
            name, coords, mass = self.zmatrix[0]
            self.cartesian = [[name, np.array([0., 0., 0.]), mass]]

        if isinstance(self.origin2,np.ndarray):
            name, coords, mass = self.zmatrix[1]
            self.cartesian.append([name, self.origin2, self.masses[name]])
        else:
            # Second atom
            name, coords, mass = self.zmatrix[1]
            distance = coords[0][1]
        ...
        if isinstance(self.origin3,np.ndarray):
            name, coords, mass = self.zmatrix[2]
            self.cartesian.append([name, self.origin3, self.masses[name]])
        else:
            # Third atom
            name, coords, mass = self.zmatrix[2]
...
def z_mat_to_cartesian(z_mat_input,atom1_cartesian=None,atom2_cartesian=None,atom3_cartesian=None):
    '''
    :param z_mat_input: A multi-line text, or a Example:
        C
        C	1	1.511
        H	1	1.088	2	110.5
        H	1	1.091	2	110.1	3	-108.8
        ...
        H	2	1.098	1	110.7	3	-59.865
        H	2	1.098	1	110.7	3	59.865
    :return: A text for the converted cartesian coordinate
    '''

    with open('zmatrix.dat','w') as z_mat_input_file:
        z_mat_input_file.write('\n\n')
        z_mat_input_file.write(z_mat_input)

    a = Converter()
    # print('a')
    # print(atom1_cartesian,atom2_cartesian,atom3_cartesian)
    a.read_zmatrix('zmatrix.dat',atom1_cartesian,atom2_cartesian,atom3_cartesian)
