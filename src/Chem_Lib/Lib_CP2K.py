...
class CP2K_Input:
    def __init__(self, input_file: Optional[str] = None, input_content: Union[str, list, None] = None):
        ...
        self._cell_vectors = None  # [vector-A, vector-B, vector-C]
    ...
    def coordinate(self):
        if self._coordinate is None:
            ...
            self._coordinate = Coordinates(atoms, charge, multiplicity, periodic_cell=self.cell_vectors)
...
class CP2K_Output:
    def __init__(self, output_file: Optional[str] = None, output_content: Union[str, list, None] = None):
        self.input_file = output_file

        if output_file is not None:
            with open(output_file) as input_file_object:
                self.output_lines = input_file_object.read().splitlines()

        elif isinstance(output_content, str):
            self.output_lines = output_content.splitlines()
        else:
            self.output_lines = output_content

        self._last_coordinate = None
        self._project_name = None
        self._non_PP_electrons = None


    @property
    def project_name(self):
        if self._project_name is None:
            # Notice that you can't read from the front, as the CP2K's output write is append mode, note overwrite move,
            # so one output file can have multiple starting points. One has to read the last hit.
    ...
    @property
    def non_pp_electrons(self):
        """
        Get list of actual electrons used for each element Multiwfn molden file process
        by reading the Mulliken Population Analysis
          #  Atom  Element  Kind  Atomic population                           Net charge
          1     N        1          5.052540                             -0.052540
        or the Hirshfeld Charges
              #Atom  Element  Kind  Ref Charge     Population                    Net charge
          1       N      1       5.000          5.243                        -0.243

        :return: ordered dict like: {"C":4, "O":6}
        """

        ret = OrderedDict()
        for count, line in enumerate(self.output_lines):
            if "Mulliken Population Analysis" in line:
                for table_line in self.output_lines[count:]:
                    if "!------------------------------" in table_line:
                        break
                    re_ret = re.findall(r"\d+\s+([A-Z][a-z]{0,2})\s+\d+\s+(\d+\.\d+)\s+-*\d+\.\d+\s+-*\d+\.\d+",table_line)
                    if re_ret:
                        element,charge = re_ret[0]
                        charge = round(charge)
                        if element in ret:
                            assert ret[element]==charge
                        ret[element] = charge
...
def modify_CP2K_molden_for_Multiwfn(cp2k_molden_file, cp2k_output_file=None, cp2k_input_file=None):
    """
    Process molden file as specified by Multiwfn manual 2.9.2.1
    优先从output读信息，没有的话从input
    假设output和input的文件名主干相同，output后缀cp2k，input后缀inp
    :param cp2k_input_file:
    :param cp2k_molden_file:
    :return:
    """
    ...
    assert os.path.isfile(cp2k_molden_file)

    cp2k_input = CP2K_Input(cp2k_input_file)

    cp2k_molden_file_content = open(cp2k_molden_file).read().splitlines()
    if "[Cell]" not in cp2k_molden_file_content[2]:
        ret = ['[Cell]']+cp2k_input.cell_vectors

        with open(cp2k_molden_file,"w") as cp2k_molden_file_object:
            cp2k_molden_file_object.write("\n".join(cp2k_molden_file_content[:1]+ret+cp2k_molden_file_content[1:]))
