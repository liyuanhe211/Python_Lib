...
from My_Lib_System import non_block_keyboard_interrupt, disable_keyboard_interrupt, enable_keyboard_interrupt, is_headless
from My_Lib_MachineLearning_Loss_Functions import *
from My_Lib_Plot_JSON import Curve_DataClass, Grid_DataClass, Plot_DataClass, _json_default
from My_Lib_MachineLearning_Utilities import *
...
DEFAULT_PLOT_Window_SIZE = (350, 300)
...
class Train_NN_Network:
    def __init__(self,
        ...
        # Organize JSON outputs into separate folders
        self.plots_folder = os.path.join(save_folder, "Plots")
        self.control_panel_folder = os.path.join(save_folder, "Control_Panel_History")
        os.makedirs(self.plots_folder, exist_ok=True)
        os.makedirs(self.control_panel_folder, exist_ok=True)
        
        # Track saved files by (folder, tag) to enable targeted deletion instead of folder iteration
        # Format: {(folder_path, tag): filepath}
        self.json_path_records = {}
        
        # Track files saved in Best folders by best_type_name
        # Format: {best_type_name: [list of file paths]}
        self.best_folder_files = {}
        ...
        # Note: Plot windows and Training_Control_Panel are NOT created here.
        # Train_NN_Network only outputs JSON files.  Actual GUI display is
        # handled by Training_Progress_Monitor / show_Train_NN_Network_progress.
        ...
        ########## Write metadata for monitoring system ##########
        if self.save_folder:
            self._write_window_metadata()

        ########## Start monitoring process if not headless ##########
        self.monitor_process = None
        if not is_headless() and self.save_folder:
            try:
                # Start monitoring in a separate process
                self.monitor_process = multiprocessing.Process(
                    target=show_Train_NN_Network_progress,
                    args=(self.save_folder,),
                    kwargs={'poll_interval_ms': 2000},
                    daemon=True  # Daemon process will be terminated when main process exits
                )
                self.monitor_process.start()
                print(f"[Training] Monitor process started (PID: {self.monitor_process.pid})")
                
                # Give the monitor process a moment to initialize
                time.sleep(0.5)
                
                # Check if the process is still alive
                if not self.monitor_process.is_alive():
                    print("[Training] Warning: Monitor process terminated unexpectedly during startup")
                    print("[Training] This may indicate an issue with Qt/display initialization")
                    print("[Training] Training will continue, but live monitoring will not be available")
                    self.monitor_process = None
                    
            except Exception as e:
                print(f"[Training] Warning: Failed to start monitor process: {e}")
                print(f"[Training] Exception type: {type(e).__name__}")
                import traceback
                traceback.print_exc()
                self.monitor_process = None
        ...
        while self.epoch <= self.max_epoch:
            ...
            if not is_headless() and non_block_keyboard_interrupt():
        ...
        if self.monitor_process is not None:
            try:
                # Give monitor a moment to detect the termination file
                time.sleep(0.5)
                # Check if still running
                if self.monitor_process.is_alive():
                    print(f"[Training] Waiting for monitor process to finish...")
                    self.monitor_process.join(timeout=5)
                    if self.monitor_process.is_alive():
                        print(f"[Training] Terminating monitor process...")
                        self.monitor_process.terminate()
                        self.monitor_process.join(timeout=2)
            except Exception as e:
                print(f"[Training] Warning during monitor cleanup: {e}")
    ...
    def update_info_window(self, target_folder=None):
        """Dump the latest info_dict as a JSON file for the monitor to pick up."""
        if not self.info_dicts:
            return
        folder = target_folder if target_folder else self.control_panel_folder
        tag = "Control_Panel"

        # Control_Panel is kept for every epoch (not overwritten)
        filepath = os.path.join(folder,
                                f"{tag}_[E{self.epoch:05.0f}].json")
        try:
            with open(filepath, 'w', encoding='utf-8') as f:
                json.dump(self.info_dicts[-1], f, default=_json_default, indent=4)
        except Exception as e:
            print(f"Warning: Failed to dump Control_Panel JSON: {e}")
    ...
    def save_best_model_state(self, loss_term, Ys_predict_test, filename_text):
        best_states_root = self.save_path_stem + "_Best_States"
        target_dir = os.path.join(best_states_root, loss_term)
        os.makedirs(target_dir, exist_ok=True)

        # Clean up old JSON/SVG files from previous best epoch using tracked files
        # (Best folder should only keep the current best, not accumulate all epochs)
        # Instead of iterating folder, delete only the tracked files from last save
        if loss_term in self.best_folder_files:
            for old_filepath in self.best_folder_files[loss_term]:
        ...
        # Generate plot JSONs in this folder (prevent infinite recursion by passing empty new_best_list)
        self.save_plot_JSONs(Ys_predict_test, filename_text, [], target_folder=target_dir)
        ...
        # Track all JSON/SVG files that were just saved in this Best folder
        # So we can delete them specifically next time (no folder iteration needed)
        saved_files = []
        for filename in os.listdir(target_dir):
            if filename.endswith('.json') or filename.endswith('.svg'):
                saved_files.append(os.path.join(target_dir, filename))
        self.best_folder_files[loss_term] = saved_files
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            ...
            if self.should_plot_this_epoch() or len(new_best_found_category) > 0:
                self.save_plot_JSONs(Ys_predict_test,
                                  filename_text,
                                  new_best_found_category)
            ...
            if self.should_save_this_epoch():
                save_object = {'epoch': self.epoch,
                               ...
                               'training_losses': self.training_losses,
                               'test_losses': self.test_losses}
                save_path = os.path.join(self.save_folder, f"Model_[E{self.epoch:05.0f}].pth")
    ...
    def _write_window_metadata(self):
        """
        Write metadata file declaring the expected number and tags of all plot windows.
        This allows Training_Progress_Monitor to pre-allocate window positions.
        
        Called once during __init__ before starting the monitoring process.
        """
        window_tags = []
        
        # Linear Fit window
        if self.plot_current_linear_fit:
            window_tags.append("Linear_Fit")
        
        # Loss over Epoch window
        if self.plot_loss_over_epoch:
            window_tags.append("0_Losses")
        
        # Optimization Status window (always present)
        window_tags.append("0_OptStatus")
        
        # Weighted multi-loss windows
        if self.weighted_multiple_loss_mode:
            window_tags.append("0_LossTerms")
            window_tags.append("0_Weights")
            window_tags.append("0_RawLosses")
        
        # Evaluation windows
        for name in self.non_opt_evaluation_function_names:
            safe_name = "".join(c for c in name if c.isalnum() or c in ('_', '-')).strip()
            window_tags.append(f"0_Eval_{safe_name}")
        
        # Example / test-case windows
        for window_count in range(len(self.plot_cases)):
            case_name = ""
            if window_count < len(self.plot_cases_names):
                case_name = "_" + "".join(c for c in self.plot_cases_names[window_count] if c.isalnum() or c in ('_', '-')).strip()
            window_tags.append(f"Example_{window_count + 1}{case_name}")
        ...
        metadata_path = os.path.join(self.save_folder, "_metadata.json")
        try:
            with open(metadata_path, 'w') as f:
                json.dump(metadata, f, indent=2)
            print(f"[Training] Wrote window metadata: {len(window_tags)} windows expected")
        except Exception as e:
            print(f"[Training] Warning: Failed to write window metadata: {e}")

    def save_plot_JSONs(self,
                     Ys_predict_test,
                     filename_text,
                     new_best_found_category,
                     target_folder=None):
        """
        Build Plot_DataClass objects for every active plot and dump them
        (along with the control-panel info_dict) as JSON files.

        No GUI / PyQt6 code runs here.  All actual rendering is done by
        Training_Progress_Monitor.

        Naming convention:  ``<tag>_[E<epoch>].json``
        """

        # Update Control Panel JSON
        self.update_info_window(target_folder)

        epoch_suffix = f"_[E{self.epoch:05.0f}]"
        folder = target_folder if target_folder else self.plots_folder

        def _to_list(tensor_or_list):
            """Convert a tensor (or nested list/scalar) to a plain Python list."""
            if hasattr(tensor_or_list, 'tolist'):
                return tensor_or_list.tolist()
            if isinstance(tensor_or_list, (list, tuple)):
                return [x.tolist() if hasattr(x, 'tolist') else x for x in tensor_or_list]
            return tensor_or_list

        def _dump_plot(tag, plot_data: Plot_DataClass, add_epoch_in_filename=True):
            if add_epoch_in_filename:
                filepath = os.path.join(folder, f"{tag}{epoch_suffix}.json")
            else:
                filepath = os.path.join(folder, f"{tag}.json")

            try:
                plot_data.dump_to_JSON(filepath)
            except Exception as e:
                print(f"Warning: Failed to dump JSON for {tag}: {e}")
        ...
        if self.plot_current_linear_fit:
            ...
            if callable(self.function_for_numerical_evaluation):
                test_curve_vertical = _to_list(self.function_for_numerical_evaluation(Ys_predict_test))
                test_curve_horizontal = _to_list(self.function_for_numerical_evaluation(Ys_test))
                train_curve_vertical = _to_list(self.function_for_numerical_evaluation(Ys_predict_train))
                train_curve_horizontal = _to_list(self.function_for_numerical_evaluation(Ys_train))
                plottable = True
            elif Ys_predict_test.ndim == 2 and Ys_predict_test.shape[1] == 1:
                test_curve_vertical = _to_list(Ys_predict_test.squeeze(1))
                test_curve_horizontal = _to_list(Ys_test.squeeze(1))
                train_curve_vertical = _to_list(Ys_predict_train.squeeze(1))
                train_curve_horizontal = _to_list(Ys_train.squeeze(1))
                plottable = True

            if plottable:
                all_vals = test_curve_horizontal + test_curve_vertical + train_curve_horizontal + train_curve_vertical
                linear_range_min = min(all_vals)
                linear_range_max = max(all_vals)
                ...
                curves = [
                    Curve_DataClass(X=train_curve_horizontal, Y=train_curve_vertical,
                                    plot_dot=True, dot_color=blue_color, Y_label="Training results"),
                    Curve_DataClass(X=test_curve_horizontal, Y=test_curve_vertical,
                                    plot_dot=True, dot_color=red_color, Y_label="Test results"),
                    Curve_DataClass(X=[linear_range_min, linear_range_max],
                                    Y=[linear_range_min, linear_range_max],
                                    curve_color="#BBBBBB"),
                ]
                _dump_plot("Linear_Fit", Plot_DataClass(
                    Curve_objects=curves,
                    window_size_pixel=DEFAULT_PLOT_Window_SIZE, 
                    font_size=10,
                    x_axis_label="Expected", y_axis_label="Predicted",
                    window_title=f"Current Linear Fit @ Epoch {self.epoch}",
                ))
        ...
        if self.plot_loss_over_epoch:
            ...
            loss_curves = [
                Curve_DataClass(X=curve_X, Y=list(self.training_losses),
                                curve_color=blue_color, Y_label="training loss"),
                Curve_DataClass(X=curve_X, Y=list(self.test_losses),
                                curve_color=red_color, Y_label="test loss"),
            ]
            
            # Create plot data
            plot_data = Plot_DataClass(
                Curve_objects=loss_curves,
                window_size_pixel=DEFAULT_PLOT_Window_SIZE, font_size=DEFAULT_PLOT_FONT_SIZE,
                x_axis_label="Epoch", y_axis_label="Loss",
                y_lim=(0, None),
                window_title=f"Loss over Epoch @ Epoch {self.epoch}",
            )
            
            # Dump plot
            _dump_plot("0_Losses", 
                       plot_data, 
                       add_epoch_in_filename=False)

        # ---- Optimization Status ----
        opt_curves = []
        if self.scheduler is not None and len(self.lr_history) > 0:
            LR_scale_factor = max(self.test_losses) / max(self.lr_history) * 0.6
            opt_curves.append(Curve_DataClass(
                X=list(range(1, len(self.lr_history) + 1)),
                Y=[x * LR_scale_factor for x in self.lr_history],
                curve_color=blue_color, Y_label="LR"))

        opt_curves.append(Curve_DataClass(
            X=list(range(1, self.epoch + 1)), Y=list(self.test_losses),
            curve_color=black_color, Y_label="Test Loss"))
        ...
        if self.automatic_extension and self.optimization_quality_history:
            ...
            if good_epochs:
                opt_curves.append(Curve_DataClass(
                    X=good_epochs, Y=good_vals, plot_curve=False, plot_dot=True,
                    dot_color=green_color, dot_format='.', dot_width=4, Y_label="Good Epoch"))
            if bad_epochs:
                opt_curves.append(Curve_DataClass(
                    X=bad_epochs, Y=bad_vals, plot_curve=False, plot_dot=True,
                    dot_color=red_color, dot_format='.', dot_width=4, Y_label="Bad Epoch"))

        plot_data = Plot_DataClass(
            Curve_objects=opt_curves,
            window_size_pixel=DEFAULT_PLOT_Window_SIZE, font_size=DEFAULT_PLOT_FONT_SIZE,
            x_axis_label="Epoch", y_axis_label="Scaled Test Loss / LR",
            y_lim=(0, None),
            window_title=f"Opt Status @ Epoch {self.epoch}",
        )
        _dump_plot("0_OptStatus", 
                   plot_data, 
                   add_epoch_in_filename=False)
        ...
        if self.weighted_multiple_loss_mode:
            ...
            lt_curves = []
            for i in range(len(self.loss_functions)):
                train_w = [ed[i] * w[i] for ed, w in zip(self.training_loss_terms_history, self.training_weights_history)]
                test_w = [ed[i] * w[i] for ed, w in zip(self.test_loss_terms_history, self.test_weights_history)]
                color = matplotlib_colors[i + 2]
                lt_curves.append(Curve_DataClass(X=curve_X, Y=train_w, curve_color=color, curve_format=dashed_line, Y_label=f"Weighted Loss {i+1} Train"))
                lt_curves.append(Curve_DataClass(X=curve_X, Y=test_w, curve_color=color, curve_format=solid_line, Y_label=f"Weighted Loss {i+1} Test"))
            if self.use_y_range_cap:
                lt_curves.append(Curve_DataClass(X=curve_X, Y=list(self.training_cap_loss_history), curve_color="#000000", curve_format=dashed_line, Y_label="Cap Loss Train"))
                lt_curves.append(Curve_DataClass(X=curve_X, Y=list(self.test_cap_loss_history), curve_color="#000000", curve_format=solid_line, Y_label="Cap Loss Test"))
            
            plot_data = Plot_DataClass(
                Curve_objects=lt_curves, window_size_pixel=DEFAULT_PLOT_Window_SIZE, font_size=DEFAULT_PLOT_FONT_SIZE,
                x_axis_label="Epoch", y_axis_label="Loss Terms", y_lim=(0, None),
                window_title=f"Loss Terms over Epoch @ Epoch {self.epoch}",
            )
            _dump_plot("0_LossTerms", 
                       plot_data, 
                       add_epoch_in_filename=False)
            ...
            wt_curves = []
            for i in range(len(self.loss_functions)):
                train_w = [ed[i] for ed in self.training_weights_history]
                color = matplotlib_colors[i + 2]
                wt_curves.append(Curve_DataClass(X=curve_X, Y=train_w, curve_color=color, curve_format="--", Y_label=f"Weight {i+1}"))
            
            plot_data = Plot_DataClass(
                Curve_objects=wt_curves, window_size_pixel=DEFAULT_PLOT_Window_SIZE, font_size=DEFAULT_PLOT_FONT_SIZE,
                x_axis_label="Epoch", y_axis_label="Weights", y_lim=(0, None),
                window_title=f"Weights over Epoch @ Epoch {self.epoch}",
            )
            _dump_plot("0_Weights", 
                       plot_data, 
                       add_epoch_in_filename=False)
            ...
            rl_curves = []
            for i in range(len(self.loss_functions)):
                train_raw = [ed[i] for ed in self.training_loss_terms_history]
                test_raw = [ed[i] for ed in self.test_loss_terms_history]
                color = matplotlib_colors[i + 2]
                rl_curves.append(Curve_DataClass(X=curve_X, Y=train_raw, curve_color=color, curve_format=dashed_line, Y_label=f"Raw Loss {i+1} Train"))
                rl_curves.append(Curve_DataClass(X=curve_X, Y=test_raw, curve_color=color, curve_format=solid_line, Y_label=f"Raw Loss {i+1} Test"))
            if self.use_y_range_cap:
                rl_curves.append(Curve_DataClass(X=curve_X, Y=list(self.training_cap_loss_history), curve_color="#000000", curve_format=dashed_line, Y_label="Cap Loss Train"))
                rl_curves.append(Curve_DataClass(X=curve_X, Y=list(self.test_cap_loss_history), curve_color="#000000", curve_format=solid_line, Y_label="Cap Loss Test"))
            
            plot_data = Plot_DataClass(
                Curve_objects=rl_curves, window_size_pixel=DEFAULT_PLOT_Window_SIZE, font_size=DEFAULT_PLOT_FONT_SIZE,
                x_axis_label="Epoch", y_axis_label="Raw Losses", y_lim=(0, None),
                window_title=f"Raw Losses over Epoch @ Epoch {self.epoch}",
            )
            _dump_plot("0_RawLosses", 
                       plot_data, 
                       add_epoch_in_filename=False)

        # ---- Evaluation windows ----
        curve_X = list(range(1, self.epoch + 1))
        for i, name in enumerate(self.non_opt_evaluation_function_names):
            safe_name = "".join(c for c in name if c.isalnum() or c in ('_', '-')).strip()
            y_min = min([0] + self.training_evaluations_history[i] + self.test_evaluations_history[i])
            tag = f"0_Eval_{safe_name}"
            
            eval_curves = [
                Curve_DataClass(X=curve_X, Y=list(self.training_evaluations_history[i]),
                                curve_color=blue_color, Y_label="training"),
                Curve_DataClass(X=curve_X, Y=list(self.test_evaluations_history[i]),
                                curve_color=red_color, Y_label="test"),
            ]
            
            plot_data = Plot_DataClass(
                Curve_objects=eval_curves,
                window_size_pixel=DEFAULT_PLOT_Window_SIZE, font_size=DEFAULT_PLOT_FONT_SIZE,
                x_axis_label="Epoch", y_axis_label=name,
                y_lim=(y_min, None),
                window_title=f"{name} over Epoch @ Epoch {self.epoch}",
            )
            _dump_plot(tag, 
                       plot_data, 
                       add_epoch_in_filename=False)
        ...
        # ---- Example / test-case windows ----
        if self.plot_cases:
            for window_count, (case_data, case_tag) in enumerate(zip(self.plot_cases, self.plot_cases_tags)):
                ...
                example_loss = self.evaluate_loss(
                    output_predicted.unsqueeze(0), output_known.unsqueeze(0),
                    case_tag.unsqueeze(0)).item()

                case_name = ""
                if window_count < len(self.plot_cases_names):
                    case_name = "_" + "".join(c for c in self.plot_cases_names[window_count] if c.isalnum() or c in ('_', '-')).strip()
                tag = f"Example_{window_count + 1}{case_name}"

                fig_title = ""
                if window_count < len(self.plot_cases_names):
                    fig_title = f"{self.plot_cases_names[window_count]} Case {window_count + 1} | Loss {smart_format_float(example_loss, 4)}"

                case_curves = [
                    Curve_DataClass(X=_to_list(curve_X), Y=_to_list(curve_Y_known),
                                    curve_color=blue_color, Y_label="known curve"),
                    Curve_DataClass(X=_to_list(curve_X), Y=_to_list(curve_Y_predicted),
                                    curve_color=red_color, Y_label="predicted curve"),
                ]
                
                plot_data = Plot_DataClass(
                    Curve_objects=case_curves,
                    window_size_pixel=DEFAULT_PLOT_Window_SIZE, font_size=DEFAULT_PLOT_FONT_SIZE,
                    x_axis_label="Input", y_axis_label="Output",
                    window_title=f"Example Curve {window_count + 1} @ Epoch {self.epoch}",
                    figure_title=fig_title,
                )
                
                _dump_plot(tag, plot_data)

        # ---- Best model handling ----
        if len(new_best_found_category) > 0:
            for category in new_best_found_category:
                self.save_best_model_state(category, Ys_predict_test, filename_text)
